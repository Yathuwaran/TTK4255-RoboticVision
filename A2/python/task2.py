...
rho_min   = -I_rgb.shape[1]
print(rho_min)
theta_min = 0 # Placeholder value
theta_max = theta[np.argmax(theta)] # Placeholder value
...
edges = np.stack((x, y), -1) 
theta_ = np.linspace(theta_min, theta_max, N_theta) 
theta_ = np.stack((np.cos(theta_), np.sin(theta_)))
rhos = np.dot(edges, theta_)
...
row = np.floor(N_rho * (rhos - rho_min) /
               (rho_max - rho_min)).astype(np.int)

column = np.floor(N_theta * (theta - theta_min) /
               (theta_max - theta_min)).astype(np.int)
...
rho_values = ((row+0.5) / N_rho) * (rho_max - rho_min) + rho_min
theta_values = ((col+0.5) / N_theta) * \
